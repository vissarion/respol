...
typedef CGAL::Convex_hull_d<FCKernel> 				Triangulation;
...
typedef Triangulation::Vertex_handle   				Vertex_handle_d;
typedef Triangulation::Vertex_iterator    		Vertex_iterator_d;
typedef Triangulation::Facet_handle   				Facet_handle_d;
typedef Triangulation::Facet_iterator  				Facet_iterator_d;
...
void init_normal_list(Triangulation& ppc, std::vector<Vector_d>& normal_list){
	Field vec[]={-1,0,1};
...
std::vector<Direction_d> CH_normal_vectors_d(Triangulation& CH){
...
vector<set<int> > project_lower_upper_hull(Triangulation& CH, map<vector<Field>, 
                                     int>& points_index,
...
// VERY NAIVE!!!!
void insert_new_Rvertex(Triangulation& CH, std::vector<Vector_d>& normal_list_d, Point_d new_point){
...
void compute_res(std::vector<std::vector<Field> >& pointset, map<std::vector<Field>,int>& points_index, int m, std::vector<int>& mi, vector<int>& proj, HD& dets, int& numof_triangs, int& numof_init_Res_vertices, Triangulation& CH){
...
/*
void compute_res_fast(std::vector<std::vector<Field> >& pointset, map<std::vector<Field>,int>& points_index, int m, std::vector<int>& mi, vector<int>& proj, HD& dets, int& numof_triangs, int& numof_init_Res_vertices, Triangulation& CH){
	// the lifted static convex hull
	Triangulation sCH(CD);
...
//NOT WORKING !!!!!!! 
void insert_new_Rvertex_old(Triangulation& CH, std::vector<Direction_d>& normal_list_d, Point_d new_point){
  int before = CH.number_of_vertices();
  CH.insert(new_point);
  int after = CH.number_of_vertices();
	if (after!=before){
...
Direction_d augment_CH_dim(Triangulation& CH){
	int idim=0;
