...
void
Linear_algebraCd<FT,AL>::
destructive_Gaussian_elimination(Matrix &U,
                     // return parameters
                     Matrix &L,// Matrix &U,
                     std::vector<int> &row_permutation,
                     std::vector<int> &column_permutation,
                     FT &det, int &rank, Vector &c)
{
  ...
  int i, j, k;
  int dim  = U.row_dimension(), cdim = U.column_dimension();
  // All the parameters are already initialized (as in C++)
  int sign = 1;
  // First create a copy of M into U, and set L and permutations to identity
  //U = M; 
...
typename Linear_algebraCd<FT,AL>::FT
Linear_algebraCd<FT,AL>::
destructive_determinant(Matrix &M, Matrix &L,/* Matrix &U,*/
            std::vector<int> &q, Vector &c)
{
  FT det; int rank;
  std::vector<int> cq;
  destructive_Gaussian_elimination(M, L,/* U,*/ q, cq, det, rank, c);
  return det;
}
...
typename Linear_algebraCd<FT,AL>::FT
Linear_algebraCd<FT,AL>::
destructive_determinant(Matrix &M)
{
  Matrix L/*,U*/; Vector c;
  std::vector<int> q;
  return destructive_determinant(M,L,/*U,*/q,c);
}
...
Sign
Linear_algebraCd<FT,AL>::
destructive_sign_of_determinant(Matrix &M)
{ return CGAL_NTS sign(destructive_determinant(M)); }
#endif // USE_HACKED_GAUSSIAN_ELIMINATION
